...
$ M \subset \mathbb{N} \times \mathbb{N} $,
...
\begin{figure}
    \caption{Transition system representation of a single battle}
    \label{tikz:battle}
    \begin{tikzpicture}
        ...
        \graph[use existing nodes] {
            s0 -> s1 ;
            s1 ->[edge label={attack, $1 - p$}] s2 ;
            s1 ->[edge label={attack, $ p $}] s3 ;
            s2 -> s1;
            s3 -> s4;
            s4 ->[edge label={attack}] s4;
